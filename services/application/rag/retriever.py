...
from services.application.preprocessing.dispatchers import EmbeddingDispatcher
...
class ContextRetriever:
    ...
    def search(
        ...
        query: str,
        k: int = 4,
        expand_to_n_queries: int = 4,
    ) -> list:
        query_model = Query.form_str(query)

        query_model = self._metadata_extractor.generate(query_model)
        logger.info(
            f"Successfully extracted the author_full_name = {query_model.author_user_name} from the query.",
        )

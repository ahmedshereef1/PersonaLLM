...
from .base_selenium_crawler import BaseSeleniumCrawler
from services.domain.documents import PostDocument
from services.settings import settings
from services.domain.exceptions import ImproperlyConfigured
from services.domain.exceptions import NoSuchElementException
...
class LinkedinCrawler(BaseSeleniumCrawler):
    ...
    def extract(self, link, **kwargs) -> None:
        ...
        if self.model.link is not None:
            ...
            logger.info(f"Starting scraping data for the profile: {link}")
        ...
        logger.debug(f"Scraped profile data: {data}")
        ...
        try:
            ...
            logger.warning("Posts button not found")

        # Scrolling and scraping posts.
        self.scroll_page()
        soup = BeautifulSoup(self.driver.page_source, "html.parser")
        post_element = soup.find_all(
            "div",
        ...
        posts = self._extract_posts(post_element, post_images)
    ...
    def _get_page_content(self, url: str) -> BeautifulSoup:
        """
        Retrieve the page content from the given url.
        """
        self.driver.get(url)
        time.sleep(5)
    ...
    def _scrape_section(self, soup: BeautifulSoup, *args, **kwargs) -> str:
        """
        Scrap a specific section from the LinkedIn profile.
        Example: Scrap the 'About' section
        """

...
from services.domain.inference import DeploymentStrategy
...
class SagemakerHuggingfaceStrategy(DeploymentStrategy):
    ...
    def deploy(
        ...
        logger.info(
            f"Deployment parameter is: no. of replicas: {settings.COPIES}, no. of gpus: {settings.GPUS}, instance type: {settings.GPU_INSTANCE_TYPE}"
        )
        ...
        try:
            ...
            logger.error(f"Error happned during the deployment: {e}")
            raise


class DeploymentService:
    def __init__(self, resource_manger):
        """
        ...
        self.resource_manger = resource_manger
    ...
    def deploy(
        ...
        try:
            ...
            if self.resource_manger.endpoint_config_exists(
                endpoint_config_name=endpoint_config_name
            ):
                logger.info(
    ...
    def prepare_and_deploy_model(
        ...
        huggingface_model = HuggingFaceModel(
            role=role_arn, image_uri=llm_image, env=config
        )

...
from services.infrastructure.database_connection.qdrant import connection
...
class VectorBaseDocument(BaseModel, Generic[T], ABC):
    ...
    def bulk_insert(cls: Type[T], documents: list["VectorBaseDocument"]) -> bool:
        try:
            ...
            logger.info(f"Collection '{cls.get_collection_name()} doesn't exist.'")
    ...
    @classmethod
    def search(cls: Type[T], limit: int = 10, query_vector=list, **kwargs) -> list[T]:
        try:
    ...
    @classmethod
    def get_collection_name(cls: Type[T]) -> DataCategory:

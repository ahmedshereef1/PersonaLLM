...
from services.domain.documents import (
    UserDocument,
    Document,
    ArticleDocument,
    PostDocument,
...
def query_data_warehouse(
    author_user_name: list[str],
) -> Annotated[list, "raw_documents"]:
    documents = []
    authors = []

    for author_user in author_user_name:
        logger.info(f"Querying data warehouse for user: {author_user}")

        first_name, last_name = utils.split_user_name(author_user)
...
def _get_metadata(documents: list[Document]) -> dict:
    ...
    for document in documents:
        ...
        metadata[collection]["authors"].append(document.author_user_name)
...
def __fetch_articles(user_id) -> list[NoSQLBaseDocument]:
    result = ArticleDocument.bulk_find(author_id=user_id)
    logger.info(f"Articles result type: {type(result)}")
    logger.info(f"Articles result: {result}")
    return result
